...
**Creating a scope's row is serialized, because the `UPDATE` alone does not cover it.** The same NULL-distinctness that breaks the conflict branch also means two concurrent *first* adjustments for a NULL-tenant scope both miss the `UPDATE`, both insert without ever conflicting, and collapsing the resulting duplicates has to discard one of the deltas — permanently, since neither row can be reconstructed from the other. Verified against Postgres 17: six concurrent first adjustments left `base_count = 1`.

The rowless branch therefore takes a scope-keyed `pg_advisory_xact_lock` and **re-runs the increment inside it** before inserting. Whichever caller loses the race blocks on the lock until the winner's transaction commits, then finds the row and increments it instead of creating a rival. `pg_advisory_xact_lock` is released by the transaction, so it cannot leak the way a session lock does when a caller throws mid-flight; when the caller already supplied a transaction the lock is taken on it and held until that transaction commits, which is exactly the window during which its uncommitted row is invisible to everyone else. `applyCoverageAdjustments` sorts its aggregated scopes by scope key so that two transactions adjusting the same pair of new scopes cannot take those locks in opposite orders and deadlock. Holding the lock makes a duplicate impossible, so the insert seeds rather than increments and no collapse follows it; its `ON CONFLICT DO NOTHING` is only a backstop against a writer that bypasses this path.

Collapsing duplicates on the incrementing path now **folds** the removed rows' counters into the row that is kept, rather than deleting them outright. On the absolute-total path (`upsertCoverageRow`) a duplicate carries no information the keeper lacks, so deleting is right there; on the incrementing path a duplicate *is* an accumulated total, and dropping it loses every adjustment that landed on it. Any duplicate that reaches this code pre-dates the serialized initialization — it comes from a database that ran the NULL-distinct upsert — so folding repairs that history instead of compounding it.

In steady state the adjustment is still one statement. The other statements in `incrementCoverageRow` are recovery paths that stop firing once a scope settles: the legacy NULL-organization fold runs only for global-organization scopes, the locked initialization only until the scope's row exists, and the duplicate fold only when the scope somehow holds more than one row.
...
- `packages/core/src/modules/query_index/__integration__/TC-QIDX-5613-coverage-accumulation.spec.ts` — runs the real `applyCoverageAdjustments` against the ephemeral shard's Postgres and asserts the resulting totals: three sequential `+1` adjustments accumulate to `3` in a tenant-scoped, a NULL-tenant, and a NULL-tenant/NULL-organization scope; five overlapping adjustments add `5` to an existing counter without fanning out into duplicate rows; and — starting from **no row at all**, which is the branch the unique constraint cannot protect — six concurrent first adjustments on a NULL-tenant scope settle on one row carrying all six. This is the layer that distinguishes "incremented" from "overwritten"; the rows belong to a throwaway `entity_type` unique per run, so no other suite's coverage is touched.
